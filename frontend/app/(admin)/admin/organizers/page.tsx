...
export default function AdminOrganizersPage() {
  ...
  // Note: we mirror checkers page structure but only implement listing + optional create link.
  // Admin create/delete endpoints for organizers are not currently exposed in `app/_lib/api.ts`.
  ...
  const loadOrganizers = React.useCallback(async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(err, "Unable to load organizers."));
    } finally {
      setLoading(false);
    }
  }, [canAccess]);
    ...
    return (
      <div className="mx-auto max-w-4xl px-6 py-8">
        <Card className="space-y-4">
          ...
          <h1 className="text-3xl font-semibold text-[var(--color-ink)]">Organizer management belongs to admin accounts.</h1>
          <p className="text-sm leading-6 text-[var(--color-ink-muted)]">Sign in with an admin role to create or manage organizer users.</p>
          <Link href="/login">
    ...
    <div className="mx-auto max-w-7xl px-3 py-4 sm:px-6">
      <div className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {/* Create page route (to be implemented separately) */}
      ...
      <div className="mt-6 grid gap-6">
        <div className="space-y-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[var(--color-ink-muted)]">Organizer Directory</p>
              <h2 className="mt-2 text-xl font-semibold text-[var(--color-ink)] sm:text-2xl">{organizers.length} Organizers</h2>
            </div>

            <Button
              variant="outline"
              size="sm"
              onClick={() => void loadOrganizers()}
          ...
          {loading ? (
            ...
            <Card className="flex flex-col items-center justify-center py-16 text-center sm:py-20">
              ...
              <p className="mt-2 text-sm text-[var(--color-ink-muted)]">Create an organizer account to start managing events.</p>
            </Card>
          ) : (
                      ...
                      >
                        ...
                        <td className="px-4 py-4 text-right sm:px-6">
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              if (window.confirm(`Delete ${organizer.name}?`)) {
